...
import org.o2o.dao.ProductImgDao;
...
import org.o2o.util.ImageUtil;
import org.o2o.util.PageCalculator;
import org.o2o.util.PathUtil;
...
public class ProductServiceImpl implements ProductService{
	...
	/**
	 * 1.若缩略图参数有值，则处理缩略图
	 * 2若原先存在缩略图则先删除在添加新的缩略图，之后获取缩略图相对路径并赋值给product
	 * 3.若商品详情图列表参数优质，对商品详情图片列表进行相同的操作
	 * 4.将tb_product_img下面的该商品原先的商品详情图记录全部清除
	 * 5.更新tb_product及tb_product_img的信息
	 */
	@Override
	@Transactional
	public ProductExecution modifyProduct(Product product, ImageHolder thumbnail,
		...
		if(product!=null&&product.getShop()!=null&&product.getShop().getShopId()!=null) {
			...
			//默认设置为上架的状态
//			product.setEnableStatus(1);
			//若商品缩略图不为空则而且原有缩略图不为空则删除原有缩略图并添加
			if(thumbnail!=null) {
	...
	private void addProductImgList(Product product, List<ImageHolder> productImgHolderList) {
		...
		if(productImgList.size()>0) {
			try {
				...
				if(effectedNum<=0) {
					throw new ProductOperationException("船舰商品详情图失败");
				}
			}catch(Exception e) {
